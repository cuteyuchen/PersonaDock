...
CAPABILITIES: tuple[WebCapability, ...] = (
    ...
    WebCapability("persona.init", "新建 Persona", "persona", "init", "/api/v1/personas", "#/personas/new", "planned"),
    WebCapability("persona.distill", "蒸馏 Persona", "persona", "distill", "/api/v1/ai/distill", "#/ai-studio", "planned", runs_as_job=True),
    WebCapability("persona.validate", "验证 Persona", "persona", "validate", "/api/v1/personas/{persona_id}/validation", "#/personas/{persona_id}/tests", "planned", runs_as_job=True),
    WebCapability("persona.migrate", "迁移 Canonical Schema", "persona", "migrate", "/api/personas/{persona_id}/migrate-v3", "#/personas/{persona_id}/editor", "legacy", destructive=True, supports_preview=True),
    WebCapability("persona.diff", "语义差异", "persona", "diff", "/api/personas/diff", "#/diff", "legacy", supports_preview=True),
    WebCapability("persona.test", "场景测试", "persona", "test", "/api/personas/{persona_id}/tests", "#/personas/{persona_id}/tests", "legacy", runs_as_job=True),
    WebCapability("persona.list", "Persona 列表", "persona", "persona list", "/api/personas", "#/personas", "ready"),
    WebCapability("persona.show", "Persona 详情", "persona", "persona show", "/api/personas/{persona_id}", "#/personas/{persona_id}", "ready"),
    WebCapability("persona.register", "注册现有工程", "persona", "persona register", "/api/v1/personas/register", "#/personas/register", "planned"),
    WebCapability("persona.export", "导出 Persona", "persona", "export", "/api/personas/{persona_id}/exports", "#/personas/{persona_id}/packages", "legacy", runs_as_job=True),
    ...
    WebCapability("runtime.discover", "发现 Runtime", "runtime", "discover", "/api/discover", "#/runtimes", "ready", runs_as_job=True),
    WebCapability("runtime.instances", "Runtime 列表", "runtime", "instances", "/api/instances", "#/runtimes", "ready"),
    WebCapability("runtime.adopt", "接管已有 Runtime", "runtime", "adopt", "/api/adoptions", "#/runtimes", "legacy", destructive=True, supports_preview=True, runs_as_job=True),
    ...
    WebCapability("revision.list", "Revision 历史", "revision", None, "/api/v1/personas/{persona_id}/revisions", "#/personas/{persona_id}/revisions", "planned"),
    WebCapability("revision.restore", "恢复 Revision", "revision", None, "/api/v1/personas/{persona_id}/revisions/{revision_id}/restore", "#/personas/{persona_id}/revisions", "planned", destructive=True, supports_preview=True),
